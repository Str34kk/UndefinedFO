...
static LPCTSTR lpszAppName = "UndefinedFO";
...
void kulaElipsa(double x, double y, double z, double width, double height)
{
	double m_x, m_y, m_z;
	...
	for (horizontalArc = 0; horizontalArc <= GL_PI * 2; horizontalArc += rate)
	{
		...
		for (vericalArc = 0; vericalArc <= GL_PI; vericalArc += rate)
		{
			...
			m_x = width * (sin(vericalArc) * sin(horizontalArc));
			m_xNext = width * (sin(vericalArc) * sin(horizontalArc + rate));

			m_z = width * (sin(vericalArc) * cos(horizontalArc));
			m_zNext = width * (sin(vericalArc) * cos(horizontalArc + rate));

			glVertex3d(x + m_x, y + m_y, z + m_z);
			glVertex3d(x + m_xNext, y + m_y, z + m_zNext);
		}
		glEnd();
	}
}
